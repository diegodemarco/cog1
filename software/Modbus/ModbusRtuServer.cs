    ...
    public class ModbusRtuServer : ModbusServer, IDisposable
    {
        ...
        public ModbusRtuServer(string portName, int baudRate)
        {
            ...
            try
            {
                ...
                interFrameDelay = (int)(50000.00 / baudRate); 
                if (interFrameDelay < MIN_INTER_FRAME_DELAY)
                    interFrameDelay = MIN_INTER_FRAME_DELAY;
            }
            catch
            {
                serialPort.Dispose();
        ...
        private void DoInterFrameDelay()
        {
        ...
        private bool ReadChars(byte[] buffer, ref int offset, int byteCount)
        {
            ...
            while (sw.ElapsedMilliseconds < timeout)
            {
                ...
                {
                    var bytesRead = serialPort.Read(buffer, offset, byteCount);
                    offset += bytesRead;
                    byteCount -= bytesRead;
                    //Console.WriteLine($"Success: received {bytesRead} bytes");
                    if (byteCount == 0)
                        return true;
                }
                Thread.Sleep(10);
        ...
        protected override bool ExchangeData(object conn, byte[] requestData, out byte[] responseData)
        {
            ...
            GetChecksum(requestData, 0, len, out var chk0, out var chk1);
            var newArray = new byte[len + 2];
            Array.Copy(requestData, newArray, len);
            newArray[len] = chk0;
            newArray[len + 1] = chk1;

            //Console.WriteLine($"Frame to be sent with checksum is [{Utils.BytesToHex(newArray, " ")}]");
            ...
            //{
            //    Console.WriteLine($"Could not read echo => [{Utils.BytesToHex(buffer, index)}]");
            //    return false;
            //}
            //index = 0;
        ...
        private static UInt16[] wCRCTable =
        {
        ...
        private void GetChecksum(byte[] arr, int offset, int len, out byte chk0, out byte chk1)
        {
            if (len < 3)
            {
                chk0 = 0;
                chk1 = 0;
                Console.WriteLine("Too short for checksum calculation");
                return;
            }
            ...
            while (len-- != 0)
            {
                nTemp = (byte)(arr[offset++] ^ wCRCWord);
                wCRCWord >>= 8;
                wCRCWord = (UInt16)(wCRCWord ^ wCRCTable[nTemp]);
            }

            chk1 = (byte)(wCRCWord >> 8);
        ...
        private bool StripChecksum(byte[] arr, int len, out byte[] data)
        {
            if (len < 3)
            {
                SetErrorInfoBadChecksum();
                data = Array.Empty<byte>();
                return false;
            }

            // Verify checksum
            GetChecksum(arr, 0, len - 2, out var chk0, out var chk1);
            if (arr[len - 2] != chk0 || arr[len - 1] != chk1)
            {
                Console.WriteLine($"Incorrect checksum: [{Utils.BytesToHex(arr, len, " ")}]");
                SetErrorInfoBadChecksum();
                data = Array.Empty<byte>();
                return false;
            }

            // Checksum is Ok. Return data
            data = new byte[len - 2];
