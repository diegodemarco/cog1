...
## What is cog1?
cog1 is an open-source industrial IoT gateway (hardware and software) built as a motherboard hosting an [Orange Pi Zero 3]([url](http://www.orangepi.org/html/hardWare/computerAndMicrocontrollers/details/Orange-Pi-Zero-3.html)) linux SBC (Wiki [here]([url](http://www.orangepi.org/orangepiwiki/index.php/Orange_Pi_Zero_3))). Its software uses the .net core framework (currently at version 8) to provide the services running in the hardware, as well as the back-end web services for the user interface (not started yet), which will be developed in Angular.
...
- Four digital inputs (isolated, using two [ISO 1212]([url](https://www.ti.com/lit/ds/symlink/iso1212.pdf)) chips from TI).
- Four digital outputs (relays, varistor-protected).
- Four analog voltage inputs (0-10 V, non-isolated), protected up to 48 VDC.
- Four analog current inputs (0-20 mA, non isolated), protected up to 100 mA.
...
- Allow running javascript code using [Jint]([url](https://github.com/sebastienros/jint)) to make it possible to compute complex variables, do edge data-processing, etc.
